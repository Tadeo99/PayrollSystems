 ...
 * @since Rep v1..0
 */
public class GenericPagoBatchDAOImpl<K, T> extends GenericOperacionDAOImpl<K, T> {

	private Logger log = LoggerFactory.getLogger(GenericPagoBatchDAOImpl.class);
	/** El objeto entity manager. */
	@PersistenceContext(unitName = ConfiguracionEntityManagerUtil.PU_PAGO_BATCH)
	private EntityManager entityManager;

	public GenericPagoBatchDAOImpl() {
		// Constructor
	}

	@Override
	public Connection getConexionDS() throws NamingException, SQLException {
		return getConexionDSPer("java:jboss/datasources/pagoDS");
	}
