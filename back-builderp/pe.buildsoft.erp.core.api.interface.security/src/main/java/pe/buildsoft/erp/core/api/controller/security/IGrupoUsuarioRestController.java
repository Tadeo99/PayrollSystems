...
@Tag(name = "/grupoUsuario", description = "Security Service")
...
public interface IGrupoUsuarioRestController {

	@POST
	Response crear(@Context HttpHeaders headers,GrupoUsuarioDTO obj);

	@PUT
	@Path("/{id}")
	Response modificar(@Context HttpHeaders headers,GrupoUsuarioDTO obj);

	@GET
	@Path("/{id}")
	Response eliminar(@Context HttpHeaders headers,@PathParam("id") Long id);

	@GET
	@Path("/{id}")
	Response finById(@Context HttpHeaders headers,@PathParam("id") Long id);
