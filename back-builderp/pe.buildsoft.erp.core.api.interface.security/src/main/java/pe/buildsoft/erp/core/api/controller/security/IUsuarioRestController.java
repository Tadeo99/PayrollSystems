 ...
 * @since SIAA-CORE 2.1
 */
@Tag(name = "/usuario", description = "Security Service")
...
public interface IUsuarioRestController {
	...
	@POST
	Response crear(UsuarioDTO obj);

	@PUT
	@Path("/{id}")
	Response modificar(UsuarioDTO obj);
	...
	@GET
	@Path("/{userName}")
	Response finById(@PathParam("userName") String userName);
	 ...
	 * @POST
	 * 
	 * @Path("/integracionUsuario") Response integracionUsuario(UsuarioDTO obj);
	 * 
	 * @POST
	 * 
	 * @Path("/eliminarIntegracionUsuario") Response
	 * eliminarIntegracionUsuario(UsuarioDTO obj);
	 */
}