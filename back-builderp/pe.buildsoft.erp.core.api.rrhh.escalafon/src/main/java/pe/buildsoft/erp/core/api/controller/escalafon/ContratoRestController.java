...
/**
 * La Class ContratoRestController.
 * <ul>
...
public class ContratoRestController extends GenericServiceRestImpl implements IContratoRestController {

	private Logger log = LoggerFactory.getLogger(ContratoRestController.class);

	@Inject
	private EscalafonAppServiceLocal servicioApp;

	@POST
	@Path("/{idPersonal}/contrato")
	public Response crear(@PathParam("idPersonal") String idPersonal, ContratoDTO obj) {
		return controladorAccion(idPersonal, obj, AccionType.CREAR);
	}

	@PUT
	@Path("/{idPersonal}/contrato/{id}")
	public Response modificar(@PathParam("idPersonal") String idPersonal, ContratoDTO obj) {
	...
	public Response eliminar(@PathParam("idPersonal") String idPersonal, @PathParam("id") String id) {
		var obj = new ContratoDTO();
		obj.setIdContrato(id);
		return controladorAccion(idPersonal, obj, AccionType.ELIMINAR);
	}

	private Response controladorAccion(String idPersonal, ContratoDTO obj, AccionType accionType) {
		var resultado = new RespuestaWSVO<ContratoDTO>();
		try {
			obj.setPersonal(new PersonalDTO());
			obj.getPersonal().setIdPersonal(idPersonal);
			resultado.setObjetoResultado(servicioApp.controladorAccionContrato(obj, accionType));
		} catch (Exception e) {
	...
	public Response finById(@PathParam("idPersonal") String idPersonal, @PathParam("id") String id) {
		var filtro = new ContratoDTO();
		filtro.setIdContrato(id);
		return controladorAccion(idPersonal, filtro, AccionType.FIND_BY_ID);
	}

	@GET
	@Path("/{idPersonal}/contratos")
	public Response paginador(@PathParam("idPersonal") String idPersonal, @Context HttpHeaders headers,
		...
		try {
			...
			if (filtro.getStartRow() == 0 || resultado.getContador() == 0)
				resultado.setContador(servicioApp.contarListarContrato(filtro));
			if (resultado.isData() || filtro.getStartRow() > 0) {
				resultado.setListaResultado(servicioApp.listarContrato(filtro));
			}
		} catch (Exception e) {
