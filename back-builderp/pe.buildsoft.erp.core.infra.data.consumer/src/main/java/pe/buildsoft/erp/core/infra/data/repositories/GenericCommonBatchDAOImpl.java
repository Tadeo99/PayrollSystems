 ...
 * @since Rep v1..0
 */
public class GenericCommonBatchDAOImpl<K, T> extends GenericOperacionDAOImpl<K, T> {

	private Logger log = LoggerFactory.getLogger(GenericCommonBatchDAOImpl.class);

	/** El objeto entity manager. */
	@PersistenceContext(unitName = ConfiguracionEntityManagerUtil.PU_COMMON_BATCH)
	private EntityManager entityManager;

	public GenericCommonBatchDAOImpl() {
		// Constructor
	}

	@Override
	public Connection getConexionDS() throws NamingException, SQLException {
		return getConexionDSPer("java:jboss/datasources/commonDS");
	}
