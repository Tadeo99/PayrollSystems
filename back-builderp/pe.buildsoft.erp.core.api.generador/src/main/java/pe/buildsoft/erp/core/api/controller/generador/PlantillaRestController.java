...
/**
 * La Class PlantillaRestController.
 * <ul>
...
public class PlantillaRestController extends GenericServiceRestImpl implements IPlantillaRestController {
	...
	public Response crear(@PathParam("idTecnologia") String idTecnologia, PlantillaDTO obj) {
		return controladorAccion(idTecnologia, obj, AccionType.CREAR);
	}

	@PUT
	@Path("/{idTecnologia}/plantilla/{id}")
	public Response modificar(@PathParam("idTecnologia") String idTecnologia, PlantillaDTO obj) {
	...
	public Response eliminar(@PathParam("idTecnologia") String idTecnologia, @PathParam("id") String id) {
		var obj = new PlantillaDTO();
		obj.setIdPlantilla(id);
		return controladorAccion(idTecnologia, obj, AccionType.ELIMINAR);
	}

	private Response controladorAccion(String idTecnologia, PlantillaDTO obj, AccionType accionType) {
		var resultado = new RespuestaWSVO<>();
		try {
			obj.setTecnologia(new TecnologiaDTO());
			obj.getTecnologia().setIdTecnologia(idTecnologia);
			resultado.setObjetoResultado(servicioApp.controladorAccionPlantilla(obj, accionType));
		} catch (Exception e) {
	...
	public Response finById(@PathParam("idTecnologia") String idTecnologia, @PathParam("id") String id) {
		var filtro = new PlantillaDTO();
		filtro.setIdPlantilla(id);
		return controladorAccion(idTecnologia, filtro, AccionType.FIND_BY_ID);
	}

	@GET
	@Path("/{idTecnologia}/plantillas")
	public Response paginador(@PathParam("idTecnologia") String idTecnologia, @Context HttpHeaders headers,
			@Context UriInfo info) {
		var resultado = new RespuestaWSVO<PlantillaDTO>();
		try {
			var filtro = to(info);
			filtro.setId(idTecnologia);
			if (filtro.getStartRow() == 0 || resultado.getContador() == 0)
				resultado.setContador(servicioApp.contarListarPlantilla(filtro));
			if (resultado.isData() || filtro.getStartRow() > 0) {
				resultado.setListaResultado(servicioApp.listarPlantilla(filtro));
			}
		} catch (Exception e) {
			parsearResultadoError(e, resultado);
		}
