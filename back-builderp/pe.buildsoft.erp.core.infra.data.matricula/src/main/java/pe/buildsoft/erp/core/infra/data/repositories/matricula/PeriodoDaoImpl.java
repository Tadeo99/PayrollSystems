 ...
 */
@Stateless
public class PeriodoDaoImpl extends GenericMatriculaDAOImpl<Long, Periodo> implements PeriodoDaoLocal {

	/*
	 * (non-Javadoc)
	 * 
	 * @see pe.com.builderp.core.service.matricula.dao.local.PeriodoDaoLocal#
	 * listarPeriodo(pe.com.builderp.core.service.matricula.model.jpa.Periodo)
	 */
	@Override
	public List<Periodo> listar(BaseSearch filtro) {
	...
	private Query generarQuery(BaseSearch filtro, boolean esContador) {
		...
		if (!StringUtil.isNullOrEmpty(filtro.getSearch())) {
			...
			if (!StringUtil.isNullOrEmptyNumeric(filtro.getTipo())) {
	 ...
	 * 
	 * @see
	 * pe.com.builderp.core.service.matricula.dao.local.PeriodoDaoLocal#contarListar
	...
	public Long generarId() {
		var resultado = 1L;
		var query = createQuery("select max(o.idPeriodo) from Periodo o", null);
	...
	public Periodo find(Periodo periodo) {
		...
		var jpaql = new StringBuilder();
		jpaql.append(" from Periodo o  where 1=1 ");
		if (!StringUtil.isNullOrEmpty(periodo.getDescripcion())) {
			ejecutarQuery = true;
			jpaql.append(" and upper(o.descripcion)=upper(:descripcion)");
			parametros.put("descripcion", periodo.getDescripcion());
		}
		if (!StringUtil.isNullOrEmpty(periodo.getTipo())) {
			ejecutarQuery = true;
			jpaql.append(" and o.tipo =:tipo");
			parametros.put("tipo", periodo.getTipo());
		}
		if (ejecutarQuery) {
			var query = createQuery(jpaql.toString(), parametros);
			List<Periodo> listaObj = query.getResultList();
