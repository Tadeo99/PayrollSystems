...
public class AlumnoRestController extends GenericServiceRestImpl implements IAlumnoRestController {

	private Logger log = LoggerFactory.getLogger(IAlumnoRestController.class);
	...
	@POST
	public Response crear(@Context HttpHeaders headers, AlumnoDTO obj) {
	...
	@PUT
	@Path("/{id}")
	public Response modificar(@Context HttpHeaders headers, AlumnoDTO obj) {
	...
	private Response controladorAccion(AlumnoDTO obj, AccionType accionType) {
		RespuestaWSVO<AlumnoDTO> resultado = new RespuestaWSVO<>();
		try {
			resultado.setObjetoResultado(servicioApp.controladorAccionAlumno(obj, accionType));
		} catch (Exception e) {
	...
	public Response paginador(@Context HttpHeaders headers, @Context UriInfo info) {
		...
		try {
			...
			if (filtro.getStartRow() == 0)
				resultado.setContador(servicioApp.contarListarAlumno(filtro));

			if (resultado.isData() || filtro.getStartRow() > 0) {
				resultado.setListaResultado(servicioApp.listarAlumno(filtro));
			}
		} catch (Exception e) {
