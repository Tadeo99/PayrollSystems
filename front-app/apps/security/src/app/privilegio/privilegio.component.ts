...
import { Privilegio } from "./privilegio.types";
...
export class PrivilegioComponent extends BaseComponent<Privilegio> implements OnInit, AfterViewInit, OnDestroy {
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificar(this.frmGroup.value, this.frmGroup.value.idPrivilegio).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: Privilegio) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idPrivilegio);
			}
		});
	}
