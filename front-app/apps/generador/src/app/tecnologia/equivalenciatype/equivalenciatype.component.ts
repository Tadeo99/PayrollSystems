...
import { EquivalenciaTypeService } from './equivalenciatype.service';
...
import {
  ...
  EstadoGeneralState,
  ListaItemType,
} from '@ng-mf/shared/service/comun';
import { merge, takeUntil } from 'rxjs';
import { bsAnimations } from '@ng-mf/shared/bs';
/**
 * La Class ContratoComponent.
 * <ul>
...
@Component({
  selector: 'ng-mf-bs-equivalenciatype',
  templateUrl: './equivalenciatype.component.html',
  styleUrls: ['./equivalenciatype.component.scss'],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
  animations: bsAnimations,
})
export class EquivalenciaTypeComponent
  extends BaseComponent<ConfigTypeEquivalencia>
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  constructor(
    ...
    private service: EquivalenciaTypeService,
  ...
  private crearFormulario(): void {
    this.frmGroup = this.fb.group({
      idEquivalenciaType: [null],
      tecnologiaEquivalente: this.fb.group({
        idTecnologia: [null],
      }),
      type: [null],
      typeAtribute: [null],
  ...
  public async cargarCombo() {
	...
	//paramsTemp = paramsTemp.set("tecnologiaBackEnd", 0);
	//paramsTemp = paramsTemp.set("tecnologiaFrontEnd", 0);
	await this.commonService.obtenerListaSelectItemVOMap(this.service.modulo, paramsTemp);
	this.listaTecnologia = this.commonService.getListaSelectVOItem("tecnologia");
	//this.listaBackEnd = this.commonService.getListaSelectVOItem("tecnologiaBackEnd");
	//this.listaFrontEnd = this.commonService.getListaSelectVOItem("tecnologiaFrontEnd");
}
      ...
      if (this.accionNuevo) {
        ...
        this.service
          .modificarPer(
            this.id,
            this.frmGroup.value,
            this.frmGroup.value.idEquivalenciaType
          )
          .subscribe(
            (data) => {
              if (this.isProcesoOK(data)) {
   ...
   *
   */
  public confirmarEliminar(obj: ConfigTypeEquivalencia) {
    const dialogRef = this.abrirModalConfirDialogEliminar();
    dialogRef.afterClosed().subscribe((result) => {
      if (result === 'confirmed') {
        this.eliminar(obj.idEquivalenciaType);
      }
    });
  }
   ...
   *
   */
  public find(obj: ConfigTypeEquivalencia) {
    try {
