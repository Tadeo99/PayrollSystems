...
import { Apoderado } from "./apoderado.types";
...
export class ApoderadoComponent extends BaseComponent<Apoderado> implements OnInit, AfterViewInit, OnDestroy {
	 ...
	 *
	 */
	public confirmarEliminar(obj: Apoderado) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idApoderado);
			}
		});
	}
