...
@Component({
  selector: 'app-capacidadmodaledit',
  templateUrl: './capacidadmodaledit.component.html',
  styleUrls: ['./capacidad.component.css'],
  providers: [CapacidadService]
})
export class CapacidadModalEditComponent extends BaseComponent implements  OnInit, OnChanges,AfterViewInit {
    ...
    ngOnChanges(changes: SimpleChanges) { 
		...
		if(this.data){					
			this.capacidad = this.data;		
			this.crearFormulario(this.capacidad);	
			if(this.capacidad.idCapacidad){
				this.buscarID(this.capacidad);
			}else{				
				this.nuevo();
			}
	...
	public guardar() {
		...
		try {
			this.frmGroup.value.competencia.idCompetencia = this.data.competencia.idCompetencia;
			if (this.accionNuevo) {
