...
@Component({
	selector: 'app-criterioevaluacionmodal',
	templateUrl: './criterioevaluacionmodal.component.html',
...
export class CriterioEvaluacionModalComponent extends BaseComponent implements OnInit, OnChanges, AfterViewInit {
	...
	public dataSourceBandeja = new MatTableDataSource<CriterioEvaluacion>();

	columnsToDisplay = ['competencia', 'orden', 'descripcion','peso','avre','add','remove','view'];
			...
			this.criterioEvaluacionService.paginador(this.dataProvider, this.params).subscribe(
				data => {
					if (this.isProcesoOK(data)) {
						...
						this.dataSourceBandeja = new MatTableDataSource<CriterioEvaluacion>(this.listaCriterioEvaluacion);
					}
				},
				error => {
					this.mostrarMensajeError(error);
