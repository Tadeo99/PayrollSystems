...
@Component({
	selector: 'ng-mf-bs-pabellon',
	templateUrl: './pabellon.component.html',
	styleUrls: ['./pabellon.component.scss'],
	encapsulation: ViewEncapsulation.None,
	changeDetection: ChangeDetectionStrategy.OnPush,
	animations: bsAnimations
})
export class PabellonComponent extends BaseComponent<Pabellon> implements OnInit, AfterViewInit, OnDestroy {
	...
	constructor(
		...
		private service: PabellonService,
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificar(this.frmGroup.value, this.frmGroup.value.idGrupo).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: Pabellon) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idPabellon);
			}
		});
	}
