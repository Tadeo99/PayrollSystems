...
import { BsMockApiService } from './mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class BsMockApiInterceptor implements HttpInterceptor {
    /**
     * Constructor
     */
    constructor(
        @Inject(BS_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _bsMockApiService: BsMockApiService
    ) {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        const {
            handler,
            urlParams
        } = this._bsMockApiService.findHandler(request.method.toUpperCase(), request.url);
