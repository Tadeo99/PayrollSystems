...
import { TipoDocSunatEntidadService } from "./tipodocsunatentidad.service";
...
/**
 * La Class ClasificacionComponent.
 * <ul>
 ...
 */
@Component({
	selector: 'ng-mf-bs-tipodocsunatentidad',
	templateUrl: './tipodocsunatentidad.component.html',
	styleUrls: ['./tipodocsunatentidad.component.scss'],
	encapsulation: ViewEncapsulation.None,
	changeDetection: ChangeDetectionStrategy.OnPush,
	animations: bsAnimations
})
export class TipoDocSunatEntidadComponent extends BaseComponent<TipoDocSunatEntidad> implements OnInit, AfterViewInit, OnDestroy {
	
	/** La lista tipo doc sunat entidad. */
	public listaTipoDocSunatEntidad: TipoDocSunatEntidad[] = [];

	/** La lista item select. */
	public listaTipoDocSunatEntidadSelectMap: Map<string, TipoDocSunatEntidad> = new Map<string, TipoDocSunatEntidad>();

	public listaItemTipoDocumentoEntidad : SelectItemVO[] | undefined = [];
	...
	constructor(
		...
		private service: TipoDocSunatEntidadService,
	...
	private crearFormulario(): void {
		this.frmGroup = this.fb.group({
			idTipoDocSunatEntidad: [''],
			idItemByTipoDocSunat: [0],
			idEntidad: [''],
			correla: [''],
			serie: [''],
		});
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificar(this.frmGroup.value, this.frmGroup.value.idTipoDocSunatEntidad).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	...
	public async  cargarComboItem() {
		...
		paramsTemp = paramsTemp.set(ListaItemType.TIPO_DE_COMPROBANTE,0);
		await this.commonService.obtenerListaItemSelectItemMap(paramsTemp);
		this.listaItemTipoDocumentoEntidad = this.commonService.getListaItemSelectItem(ListaItemType.TIPO_DE_COMPROBANTE);
	} 
	 ...
	 *
	 */
	public confirmarEliminar(obj: TipoDocSunatEntidad) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idTipoDocSunatEntidad);
			}
		});
	}
	 ...
	 *
	 */
	public find(obj: TipoDocSunatEntidad) {
		try {
