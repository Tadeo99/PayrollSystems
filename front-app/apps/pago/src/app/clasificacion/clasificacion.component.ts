...
@Component({
	selector: 'ng-mf-bs-clasificacion',
...
export class ClasificacionComponent extends BaseComponent<Clasificacion> implements OnInit, AfterViewInit, OnDestroy {
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificar(this.frmGroup.value, this.frmGroup.value.idClasificacion).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: Clasificacion) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idClasificacion);
			}
		});
	}
