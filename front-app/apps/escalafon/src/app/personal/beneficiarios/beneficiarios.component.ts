...
/**
 * La Class BeneficiariosComponent.
 * <ul>
...
@Component({
	selector: 'ng-mf-bs-beneficiarios',
...
export class BeneficiariosComponent extends BaseComponent<Beneficiarios> implements OnInit, AfterViewInit, OnDestroy {
	/** La lista motivoCese. */
	public listaVinculo: SelectItemVO[] | undefined = [];
	...
	public async cargarCombo() {
		...
		paramsTemp = paramsTemp.set(ListaItemType.T19_VINCULO_FAMILIAR, 0);
		await this.commonService.obtenerListaItemSelectItemMap(paramsTemp);
		this.listaVinculo = this.commonService.getListaItemSelectItem(ListaItemType.T19_VINCULO_FAMILIAR);
	}
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificarPer(this.id, this.frmGroup.value, this.frmGroup.value.idBeneficiarios).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: Beneficiarios) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idBeneficiario);
			}
		});
	}
