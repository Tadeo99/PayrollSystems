/**
 * La Class HistorialBasicoService.
 * <ul>
...
export class HistorialBasicoService extends BaseService<HistorialBasico> {

  constructor(http: HttpClient) {
    super(http);
    this.modulo = "rrhh_escalafon";
    this.url = "/historialBasico";
  }
  override eliminarPer(idPersonal: string, id: any): Observable<any> {
    this.url = "/personal/" + idPersonal + "/historialBasico";
  ...
  override paginadorPer(idPersonal: string, page: number = 0, size: number = 10, sort: string = 'nombre', order: 'asc' | 'desc' | '' = 'asc', search: string = '', paramsDinamic?: HttpParams):
    Observable<{ pagination: BasePagination; listaResultado: HistorialBasico[] }> {
    if (!paramsDinamic) {
    ...
    this.url = "/personal/" + idPersonal + "/historialBasicos";
    return this.http.get<{ pagination: BasePagination; listaResultado: HistorialBasico[] }>(this.modulo + this.url + '/', {
