/**
 * La Class HistorialCargoAreaService.
 * <ul>
...
export class HistorialCargoAreaService extends BaseService<HistorialCargoArea> {

  constructor(http: HttpClient) {
    super(http);
    this.modulo = "rrhh_escalafon";
    this.url = "/historialCargoArea";
  }
  override eliminarPer(idPersonal: string, id: any): Observable<any> {
    this.url = "/personal/" + idPersonal + "/historialCargoArea";
  ...
  override paginadorPer(idPersonal: string, page: number = 0, size: number = 10, sort: string = 'nombre', order: 'asc' | 'desc' | '' = 'asc', search: string = '', paramsDinamic?: HttpParams):
    Observable<{ pagination: BasePagination; listaResultado: HistorialCargoArea[] }> {
    if (!paramsDinamic) {
    ...
    this.url = "/personal/" + idPersonal + "/historialCargoAreas";
    return this.http.get<{ pagination: BasePagination; listaResultado: HistorialCargoArea[] }>(this.modulo + this.url + '/', {
