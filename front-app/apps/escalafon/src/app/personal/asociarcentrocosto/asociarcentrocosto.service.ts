/**
 * La Class AsociarCentroCostoService.
 * <ul>
...
export class AsociarCentroCostoService extends BaseService<AsociarCentroCosto> {

  constructor(http: HttpClient) {
    super(http);
    this.modulo = "rrhh_escalafon";
    this.url = "/centroCosto";
  }
  override eliminarPer(idPersonal: string, id: any): Observable<any> {
    this.url = "/personal/" + idPersonal + "/centroCosto";
  ...
  override paginadorPer(idPersonal: string, page: number = 0, size: number = 10, sort: string = 'nombre', order: 'asc' | 'desc' | '' = 'asc', search: string = '', paramsDinamic?: HttpParams):
    Observable<{ pagination: BasePagination; listaResultado: AsociarCentroCosto[] }> {
    if (!paramsDinamic) {
    ...
    this.url = "/personal/" + idPersonal + "/centroCostos";

    return this.http.get<{ pagination: BasePagination; listaResultado: AsociarCentroCosto[] }>(this.modulo + this.url + '/', {
