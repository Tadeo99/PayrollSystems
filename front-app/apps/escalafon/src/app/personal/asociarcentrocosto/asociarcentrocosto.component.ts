...
import { AsociarCentroCostoService } from "./asociarcentrocosto.service";
import { AsociarCentroCosto } from "./asociarcentrocosto.types";


/**
 * La Class AsociarCentroCostoComponent.
 * <ul>
...
@Component({
	selector: 'ng-mf-bs-asociarcentrocosto',
	templateUrl: './asociarcentrocosto.component.html',
	styleUrls: ['./asociarcentrocosto.component.scss'],
	encapsulation: ViewEncapsulation.None,
	changeDetection: ChangeDetectionStrategy.OnPush,
	animations: bsAnimations
})
export class AsociarCentroCostoComponent extends BaseComponent<AsociarCentroCosto> implements OnInit, AfterViewInit, OnDestroy {
	/** La lista TipoAsociarCentroCosto. */
	public listaCentroCosto: SelectItemVO[] | undefined = [];
	...
	constructor(
		...
		private service: AsociarCentroCostoService,
		...
		this.SORT_NAME = 'centroCosto.nombre';
	}
	...
	private crearFormulario(): void {
		this.frmGroup = this.fb.group({
			idAsociarCentroCosto: [null],
			centroCosto: this.fb.group({
				idCentroCosto: [null],
				nombre: [null]
			}),
			fechaInicio: [null],
	...
	public async cargarCombo() {
		...
		paramsTemp = paramsTemp.set("centroCosto", 0);
		await this.commonService.obtenerListaSelectItemVOMap(this.service.modulo, paramsTemp);
		this.listaCentroCosto = this.commonService.getListaSelectVOItem("centroCosto");
	}
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificarPer(this.id, this.frmGroup.value, this.frmGroup.value.idAsociarCentroCosto).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: AsociarCentroCosto) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idAsociarCentroCosto);
			}
		});
	}
	 ...
	 *
	 */
	public find(obj: AsociarCentroCosto) {
		try {
