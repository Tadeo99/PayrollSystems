...
export class ContratoComponent extends BaseComponent<Contrato> implements OnInit, AfterViewInit, OnDestroy {
	/** La lista TipoContrato. */
	public listaTipoContrato: SelectItemVO[] | undefined = [];
	...
	constructor(
		...
		this.SORT_NAME = 'idItemByTipoContrato';
	}
	...
	public async cargarCombo() {
		...
		paramsTemp = paramsTemp.set(ListaItemType.T12_CONTRATOS, 0);
		await this.commonService.obtenerListaItemSelectItemMap(paramsTemp);
		this.listaTipoContrato = this.commonService.getListaItemSelectItem(ListaItemType.T12_CONTRATOS);
	}
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificarPer(this.id, this.frmGroup.value, this.frmGroup.value.idContrato).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: Contrato) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idContrato);
			}
		});
	}
