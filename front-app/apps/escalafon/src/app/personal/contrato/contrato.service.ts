...
export class ContratoService extends BaseService<Contrato> {
  ...
  override eliminarPer(idPersonal: string, id: any): Observable<any> {
    this.url = "/personal/" + idPersonal + "/contrato";
  ...
  override paginadorPer(idPersonal: string, page: number = 0, size: number = 10, sort: string = 'nombre', order: 'asc' | 'desc' | '' = 'asc', search: string = '', paramsDinamic?: HttpParams):
    Observable<{ pagination: BasePagination; listaResultado: Contrato[] }> {
    if (!paramsDinamic) {
    ...
    this.url = "/personal/" + idPersonal + "/contratos";
    return this.http.get<{ pagination: BasePagination; listaResultado: Contrato[] }>(this.modulo + this.url + '/', {
