...
export class CarreraComponent extends BaseComponent<Carrera> implements OnInit, AfterViewInit, OnDestroy {
	/** La lista Institucion. */
	public listaInstitucion: SelectItemVO[] | undefined = [];
	...
	public async cargarCombo() {
		...
		paramsTemp = paramsTemp.set("institucion", 0);
		await this.commonService.obtenerListaSelectItemVOMap("rrhh_escalafon", paramsTemp);
		this.listaInstitucion = this.commonService.getListaSelectVOItem("institucion");
	}
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificar(this.frmGroup.value, this.frmGroup.value.idCarrera).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: Carrera) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idCarrera);
			}
		});
	}
