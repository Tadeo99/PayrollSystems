...
import { CentroCosto } from "./centrocosto.types";


/**
 * La Class CentroCostoComponent.
 * <ul>
...
export class CentroCostoComponent extends BaseComponent<CentroCosto> implements OnInit, AfterViewInit, OnDestroy {
	...
	constructor(
		...
		private service: CentroCostoService,
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificar(this.frmGroup.value, this.frmGroup.value.idCentroCosto).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: CentroCosto) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idCentroCosto);
			}
		});
	}
