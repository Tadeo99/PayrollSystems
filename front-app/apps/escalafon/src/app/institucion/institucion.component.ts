...
export class InstitucionComponent extends BaseComponent<Institucion> implements OnInit, AfterViewInit, OnDestroy {
	/** La lista TipoInstitucion. */
	public listaTipoInstitucion: SelectItemVO[] | undefined = [];
	/** La lista Regimen. */
	public listaRegimen: SelectItemVO[] | undefined = [];
	...
	constructor(
		...
		this.SORT_NAME = 'idItemByTipoInstitucion';
	}
	...
	public async cargarCombo() {
		...
		paramsTemp = paramsTemp.set(ListaItemType.TIPO_INSTITUCION, 0);
		paramsTemp = paramsTemp.set(ListaItemType.TIPO_REGIMEN, 0);
		await this.commonService.obtenerListaItemSelectItemMap(paramsTemp);
		this.listaTipoInstitucion = this.commonService.getListaItemSelectItem(ListaItemType.TIPO_INSTITUCION);
		this.listaRegimen = this.commonService.getListaItemSelectItem(ListaItemType.TIPO_REGIMEN);
	}
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				...
				this.service.modificar(this.frmGroup.value, this.frmGroup.value.idInstitucion).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: Institucion) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idInstitucion);
			}
		});
	}
