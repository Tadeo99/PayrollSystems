...
/**
 * La Class TipoPlanillaComponent.
 * <ul>
...
@Component({
	selector: 'ng-mf-bs-tipoplanilla',
	templateUrl: './tipoplanilla.component.html',
	styleUrls: ['./tipoplanilla.component.scss'],
	encapsulation: ViewEncapsulation.None,
	changeDetection: ChangeDetectionStrategy.OnPush,
	animations: bsAnimations
})
export class TipoPlanillaComponent extends BaseComponent<TipoPlanilla> implements OnInit, AfterViewInit, OnDestroy {

	public listaTipoMoneda: SelectItemVO[] | undefined = [];
	...
	constructor(
		...
		private service: TipoPlanillaService,
	...
	public async cargarCombo() {
		...
		this.listaTipoMoneda = this.commonService.getListaItemSelectItem(ListaItemType.TIPO_MONEDA);
	...
	public guardar() {
		...
		try {
			if (this.accionNuevo) {
				this.service.crear(this.frmGroup.value).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
							this.guardoExito();
							this.frmGroup.get("idTipoPlanilla")?.setValue(data.objetoResultado.idTipoPlanilla);
				...
				this.service.modificar(this.frmGroup.value, this.frmGroup.value.idTipoPlanilla).subscribe(
					data => {
						if (this.isProcesoOK(data)) {
	 ...
	 *
	 */
	public confirmarEliminar(obj: TipoPlanilla) {
		const dialogRef = this.abrirModalConfirDialogEliminar();
		dialogRef.afterClosed().subscribe(result => {
			if (result === 'confirmed') {
				this.eliminar(obj.idTipoPlanilla);
			}
		});
	}
