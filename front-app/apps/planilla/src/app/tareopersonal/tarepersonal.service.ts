...
export class TareoPersonalService extends BaseService<TareoPersonal> {

  constructor(http: HttpClient) {
    super(http);
    this.modulo = "rrhh_planilla";
    this.url = "/personal/tareo";
  }
  override eliminarPer(idPersonal: string, id: any): Observable<any> {
    this.url = "/personal/" + idPersonal + "/tareo";
  ...
  override paginador(page: number = 0, size: number = 10, sort: string = 'nombre', order: 'asc' | 'desc' | '' = 'asc', search: string = '', paramsDinamic?: HttpParams):
    Observable<{ pagination: BasePagination; listaResultado: TareoPersonal[] }> {
    this.modulo = "rrhh_planilla"
    this.url = "/personal/tareos";
