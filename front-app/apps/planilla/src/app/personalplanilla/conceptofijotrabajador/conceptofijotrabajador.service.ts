/**
 * La Class ConceptoTrabajadorService.
 * <ul>
...
export class ConceptoFijoTrabajadorService extends BaseService<ConceptoFijoTrabajador> {

  constructor(http: HttpClient) {
    super(http);
    this.modulo = "rrhh_planilla";
    this.url = "/conceptoFijo";
  }
  override eliminarPer(idPersonal: string, id: any): Observable<any> {
    this.url = "/personal/" + idPersonal + "/conceptoFijo";
  ...
  override paginadorPer(idPersonal: string, page: number = 0, size: number = 10, sort: string = 'nombre', order: 'asc' | 'desc' | '' = 'asc', search: string = '', paramsDinamic?: HttpParams):
    Observable<{ pagination: BasePagination; listaResultado: ConceptoFijoTrabajador[] }> {
    this.url = "/personal/" + idPersonal + "/conceptoFijos";
