...
export class InformaOtrosIngreso5taService extends BaseService<InformaOtrosIngreso5ta> {

  constructor(http: HttpClient) {
    super(http);
    this.modulo = "rrhh_planilla";
    this.url = "/informaOtrosIngreso5ta";
  }
  override eliminarPer(idPersonal: string, id: any): Observable<any> {
    this.url = "/personal/" + idPersonal + "/informaOtrosIngreso5ta";
  ...
  override paginadorPer(idPersonal: string, page: number = 0, size: number = 10, sort: string = 'nombre', order: 'asc' | 'desc' | '' = 'asc', search: string = '', paramsDinamic?: HttpParams):
    Observable<{ pagination: BasePagination; listaResultado: InformaOtrosIngreso5ta[] }> {
    if (!paramsDinamic) {
    ...
    this.url = "/personal/" + idPersonal + "/informaOtrosIngreso5tas";
    return this.http.get<{ pagination: BasePagination; listaResultado: InformaOtrosIngreso5ta[] }>(this.modulo + this.url + '/', {
