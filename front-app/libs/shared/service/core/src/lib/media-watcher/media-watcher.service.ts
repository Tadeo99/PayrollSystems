...
import { BsConfigService } from '../config';

@Injectable()
export class BsMediaWatcherService {
    ...
    constructor(
        ...
        this._bsConfigService.config$.pipe(
            ...
            switchMap(screens => this._breakpointObserver.observe(Object.values(screens)).pipe(
                map((state) => {
                    ...
                    for (const [query] of matchingBreakpoints) {
                        ...
                        const matchingAlias = Object.entries(screens).find(([alias, q]) => q === query);
