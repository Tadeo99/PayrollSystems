...
import { BsDrawerMode, BsDrawerPosition } from './drawer.types';
import { BsDrawerService } from './drawer.service';
...
export class BsDrawerComponent implements OnChanges, OnInit, OnDestroy {
    ...
    @Input() fixed = false;
    @Input() mode: BsDrawerMode = 'side';
    @Input() name: string = this._bsUtilsService.randomId();
    @Input() opened = false;
    @Input() position: BsDrawerPosition = 'left';
    @Input() transparentOverlay = false;
    @Output() readonly fixedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly modeChanged: EventEmitter<BsDrawerMode> = new EventEmitter<BsDrawerMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<BsDrawerPosition> = new EventEmitter<BsDrawerPosition>();
    ...
    constructor(
        ...
        private _bsDrawerService: BsDrawerService,
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'bs-drawer-animations-enabled': this._animationsEnabled,
            'bs-drawer-fixed': this.fixed,
            'bs-drawer-hover': this._hovered,
            [`bs-drawer-mode-${this.mode}`]: true,
            'bs-drawer-opened': this.opened,
            [`bs-drawer-position-${this.position}`]: true
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    ngOnInit(): void {
        // Register the drawer
        this._bsDrawerService.registerComponent(this.name, this);
    }
    ...
    ngOnDestroy(): void {
        ...
        this._bsDrawerService.deregisterComponent(this.name);
    }
    ...
    private _showOverlay(): void {
        ...
        this._overlay?.classList.add('bs-drawer-overlay');

        // Add a class depending on the fixed option
        if (this.fixed) {
            this._overlay?.classList.add('bs-drawer-overlay-fixed');
        }

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay?.classList.add('bs-drawer-overlay-transparent');
        }
