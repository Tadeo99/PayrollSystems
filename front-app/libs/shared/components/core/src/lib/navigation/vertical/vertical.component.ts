...
import { bsAnimations, BsScrollbarDirective } from '@ng-mf/shared/bs';
import {
    BsNavigationItem, BsVerticalNavigationAppearance,
    BsVerticalNavigationMode, BsVerticalNavigationPosition
}
...
@Component({
    selector: 'ng-mf-bs-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: bsAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'bsVerticalNavigation'
})
export class BsVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
    ...
    @Input() appearance: BsVerticalNavigationAppearance = 'default';
    @Input() autoCollapse = true;
    @Input() inner = false;
    @Input() mode: BsVerticalNavigationMode = 'side';
    @Input() name: string = this._bsUtilsService.randomId();
    @Input() navigation!: BsNavigationItem[];
    @Input() opened = true;
    @Input() position: BsVerticalNavigationPosition = 'left';
    @Input() transparentOverlay = false;
    @Output() readonly appearanceChanged: EventEmitter<BsVerticalNavigationAppearance> = new EventEmitter<BsVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<BsVerticalNavigationMode> = new EventEmitter<BsVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<BsVerticalNavigationPosition> = new EventEmitter<BsVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl!: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<BsNavigationItem> = new ReplaySubject<BsNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<BsNavigationItem> = new ReplaySubject<BsNavigationItem>(1);
    ...
    private _bsScrollbarDirectives!: QueryList<BsScrollbarDirective>;
    private _bsScrollbarDirectivesSubscription!: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'bs-vertical-navigation-animations-enabled': this._animationsEnabled,
            [`bs-vertical-navigation-appearance-${this.appearance}`]: true,
            'bs-vertical-navigation-hover': this._hovered,
            'bs-vertical-navigation-inner': this.inner,
            'bs-vertical-navigation-mode-over': this.mode === 'over',
            'bs-vertical-navigation-mode-side': this.mode === 'side',
            'bs-vertical-navigation-opened': this.opened,
            'bs-vertical-navigation-position-left': this.position === 'left',
            'bs-vertical-navigation-position-right': this.position === 'right'
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for bsScrollbarDirectives
     */
    @ViewChildren(BsScrollbarDirective)
    set bsScrollbarDirectives(bsScrollbarDirectives: QueryList<BsScrollbarDirective>) {
        // Store the directives
        this._bsScrollbarDirectives = bsScrollbarDirectives;

        // Return if there are no directives
        if (bsScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._bsScrollbarDirectivesSubscription) {
            this._bsScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._bsScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    bsScrollbarDirectives.forEach((bsScrollbarDirective) => {
                        bsScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (!this._navigationContentEl.nativeElement.classList.contains('ps')) {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.bs-vertical-navigation-item-active');
            ...
            // Otherwise
                ...
                this._bsScrollbarDirectives.forEach((bsScrollbarDirective) => {

                    // Skip if not enabled
                    if (!bsScrollbarDirective.isEnabled()) {
                        return;
                    }

                    // Scroll to the active element
                    bsScrollbarDirective.scrollToElement('.bs-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: BsNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: BsNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay?.classList.add('bs-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay?.classList?.add('bs-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay?.classList?.add('bs-vertical-navigation-aside-overlay');
